...
// import NftStakingCardLoader from "../nft-staking-card/nft-staking-card-loader";
import NftStakingCard from "../../../../utils/api/methods-marketplace";
// import { useWallet } from "../../../../context/WalletContext";
import stakingABI from "../../../abi/pmt-token-staking.json";
import { useSelector } from "react-redux";
// Note: You'll need to replace useWalletClient with appropriate React Native wallet integration
// import { useWalletClient } from 'wagmi'; // Remove this - wagmi doesn't work in React Native
...
const NftUnStakingTabs = ({ userInfo }) => {
  ...
  const showToast = (title, description, type = "info") => {
    // You can replace this with your preferred toast library like react-native-toast-message
  ...
  const fetchNftList = useCallback(async () => {
    ...
    try {
      ...
      console.log("🚀 ~ fetchpmtList ~ response:", response);
  ...
  const handleUnStakeClick = async (tokenId, nftAddess, onClose) => {
    // Note: You'll need to adapt this for React Native wallet integration
    // This assumes you have a wallet provider that works with React Native
    if (!walletClient) {
      showToast(
        "Wallet Not Found",
    ...
    setIsUnStaking(true);
    try {
      // Note: You'll need to adapt this for React Native
      // This is a simplified version - you'll need proper wallet integration
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyTitle}>No PMT Available for Staking</Text>
      <Text style={styles.emptyDescription}>
        You don't have sufficient PMT balance to stake right now. Fund your
        wallet
      </Text>
      <TouchableOpacity
